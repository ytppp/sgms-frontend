...
`<el-alert ${titleAttr} ${typeAttr} ${descriptionAttr} ${closableAttr} ${closeTextAttr} ${centerAttr} 
 ...
 // 20230325 FYCDC-Germ  add begin
 export const transferTemplateGenerator = function(fw, formConfig) {
  const wop = fw.options ;   
  const titles = !!wop.leftTitle && !!wop.rightTitle || `:titles="[wop.leftTitle,wop.rightTitle]"`
 
  const  transferTemplate = `<el-transfer ${vModel} ${titles} ${filterable} ${size} ${clearable} ${filterable}
        ${allowCreate} ${defaultFirstOption} ${automaticDropdown} ${placeholder} ${remote} ></el-transfer>`
