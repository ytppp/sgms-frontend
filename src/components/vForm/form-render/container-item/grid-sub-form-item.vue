...
<script>
  ...
  import {deepClone, generateId, traverseFieldWidgetsOfContainer} from '@vForm/utils/util'
  ...
  export default {
    ...
    methods: {
      ...
      /**
       * 删除子表单全部行
       * @param {int} lRow  子表单行数
       */
      clearSubFormRow(lRow){        
       //let lRow = oldSubFormData.length();
        for(let i=lRow;i>0;i--){
          let oldSubFormData = this.formModel[this.widget.options.name] || []
          let deletedDataRow = deepClone(oldSubFormData[i])
          oldSubFormData.splice(i, 1)
          this.deleteFromRowIdData(i)
          this.deleteFromWidgetSchemaData(i)
