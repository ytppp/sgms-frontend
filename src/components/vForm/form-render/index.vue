...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      gResetForm(clearSubForm) {  //重置表单
        let subFormNames = Object.keys(this.subFormRefList)
        subFormNames.forEach(sfName => {
          if (clearSubForm || !!this.subFormRefList[sfName].resetSubForm) {
            this.subFormRefList[sfName].resetSubForm();
              let lRow = this.subFormRefList[sfName].getSubFormRowCount()-1;
              this.subFormRefList[sfName].clearSubFormRow(lRow); //清除子表单行
          }
        })
      ...
      /**
       * 可写字段
       * @param {string|array} widgetNames  字段名称 可以是数组或字符串
       */
      ...
      /**
       * 可写字段
       * @param {string|array} widgetNames 字段名称，字符串或级数，字符串可使用,分隔多个
       * 'a'  or 'a , b ,' or ['a','']
       */
      writableFields(widgetNames) {
        if (!!widgetNames) {
          let widgetNames_arr = [];
          if (typeof widgetNames === 'string') {
            widgetNames = widgetNames.split(',');
          }
          if (Array.isArray(widgetNames)) {
            widgetNames.forEach(wn => {
              wn=wn && wn.replace(/^(\s|\u00A0)+/,'').replace(/(\s|\u00A0)+$/,'');
              if(wn){
                this.findWidgetAndSetDisabled(wn, false)
              }              
            })
