<template>
  <div class="toolbar-container">
    <div class="left-toolbar">
      <el-button type="default" :disabled="undoDisabled" :title="i18nt('designer.toolbar.undoHint')" @click="undoHistory">
        <svg-icon icon-class="undo" /></el-button>
      <el-button type="default" :disabled="redoDisabled" :title="i18nt('designer.toolbar.redoHint')" @click="redoHistory">
        <svg-icon icon-class="redo" /></el-button>
      <el-button-group style="margin-left: 20px" v-if="designer.isShowLayoutType">
    ...
    <div class="right-toolbar" :style="{width: toolbarWidth + 'px'}">
      <div class="right-toolbar-con">
        <el-button v-if="showToolButton('clearDesignerButton')" type="primary" link @click="clearFormWidget" style="margin-top: 5px; vertical-align: center;">
          <svg-icon icon-class="el-delete" />{{i18nt('designer.toolbar.clear')}}</el-button>
        <el-button v-if="showToolButton('previewFormButton')" type="primary" link  @click="previewForm" style="margin-top: 5px; vertical-align: center;">
          <svg-icon icon-class="el-view" />{{i18nt('designer.toolbar.preview')}}</el-button>

        <el-dropdown>
            <el-button type="primary" link  style="margin-top: 5px; vertical-align: center;">
              <svg-icon icon-class="vue-sfc" /> {{i18nt('designer.toolbar.exportDropdown')}}            
            </el-button>
   
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item v-if="showToolButton('importJsonButton')" @click="importJson" >{{i18nt('designer.toolbar.importJson')}}</el-dropdown-item>
              <el-dropdown-item v-if="showToolButton('exportJsonButton')" @click="exportJson" >{{i18nt('designer.toolbar.exportJson')}}</el-dropdown-item>
              <el-dropdown-item v-if="showToolButton('exportCodeButton')" @click="exportCode" >{{i18nt('designer.toolbar.exportCode')}}</el-dropdown-item>
              <el-dropdown-item divided  v-if="showToolButton('generateSFCButton')" @click="generateSFC"><svg-icon icon-class="vue-sfc" />{{i18nt('designer.toolbar.generateSFC')}}</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>  
    ...
    <div v-if="showPreviewDialogFlag" class="" v-drag="['.drag-dialog.el-dialog', '.drag-dialog .el-dialog__header']">
      <el-dialog :title="i18nt('designer.toolbar.preview')" v-model="showPreviewDialogFlag"
        ...
        <div>
          <div id="printView" class="form-render-wrapper" :class="[layoutType === 'H5' ? 'h5-layout' : (layoutType === 'Pad' ? 'pad-layout' : '')]">
        ...
        <template #footer>
          <div class="dialog-footer">
            ...
            <el-button type="primary" @click="printPreview">{{i18nt('designer.hint.printPreview')}}</el-button>
          </div>
        </template>
      </el-dialog>
    </div>
...
<script>
  import VFormRender from '@vForm/form-render/index'
  ...
  export default {
    ...
    methods: {
      ...
      printPreview(){
        this.$refs['preForm'].setReadMode(true); //设置为只读模式，去除编辑框
        this.$nextTick(()=>{
          document.body.innerHTML = document.getElementById('printView').innerHTML;
          window.print();
          window.location.reload();
          //使用 print-js 打印格式 与预览不同，故不使用
        /* printJS({
            printable: 'printView',
            type: 'html',
            targetStyle:  ['*'],
            style:`.el-row{display: flex; flex-wrap: nowrap;white-space:nowrap;} .el-row > * { flex-grow: 1;  flex-shrink: 0;}`,
            scanStyles: false,
         })*/
        })        
      },
