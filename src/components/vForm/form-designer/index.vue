...
<template>
  <el-container class="vform-container full-height">
    <el-header class="vform-header" v-if="designerConfig.logoHeader !== false">
      <div class="float-left vform-title">
    ...
    <el-container>
      <el-aside class="side-panel" v-if="leftAsideVisible !== false">
        <widget-panel :designer="designer" />
      </el-aside>
      <div class="left-aside-toggle-bar" :style="leftAsideHide" @click="leftAsideDisplay()">
        <i class="el-icon" style="font-size: 18px;">
          <svg class="icon" width="200" height="200" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
            <path fill="currentColor" :d="leftAsideVisible ? 'M672 192L288 511.936 672 832z' : 'M384 192v640l384-320.064z'"></path>
          </svg>
        </i>
      </div>
      ...
      <div class="right-aside-toggle-bar" :style="rightAsideHide" @click="rightAsideDisplay()">
        <i class="el-icon" style="font-size: 18px;">
          <svg class="icon" width="200" height="200" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
            <path fill="currentColor" :d="rightAsideVisible ? 'M384 192v640l384-320.064z' : 'M672 192L288 511.936 672 832z'"></path>
          </svg>
        </i>
      </div>
      <el-aside v-if="rightAsideVisible !== false">
...
<script>
import WidgetPanel from '@vForm/form-designer/widget-panel/index'
import ToolbarPanel from '@vForm/form-designer/toolbar-panel/index'
import SettingPanel from '@vForm/form-designer/setting-panel/index'
import VFormWidget from '@vForm/form-designer/form-widget/index'
import { createDesigner } from '@vForm/form-designer/designer'
import {
...
export default {
  ...
  data() {
    return {
      ...
      leftAsideVisible: true, // 显示左扩展
      rightAsideVisible: true, //显示右扩展
      leftAsideHide: { left:'258px'},
      rightAsideHide:{ right: '298px' },
    }
  },
  provide() {
    return {
  ...
  methods: {
    leftAsideDisplay() {
      //console.log(this.leftAsideVisible)
      this.leftAsideVisible = !this.leftAsideVisible;
      this.leftAsideHide.left = this.leftAsideVisible ? '258px' : '-2px';
    },
    rightAsideDisplay(){
      this.rightAsideVisible=!this.rightAsideVisible;
      this.rightAsideHide.right = this.rightAsideVisible ? '298px' : '-2px';
    },
