<template>
  <div class="option-items-pane">
	...
	<!--  transfer 支持编辑选项 -->
    <el-checkbox-group v-else-if="(selectedWidget.type === 'checkbox') || ((selectedWidget.type === 'select') && selectedWidget.options.multiple) || selectedWidget.type === 'transfer'"
                    v-model="optionModel.defaultValue" @change="emitDefaultValueChange">
      <draggable tag="ul" :list="optionModel.optionItems" item-key="id"
                 v-bind="{group:'optionsGroup', ghostClass: 'ghost', handle: '.drag-option'}">
        <template #item="{ element: option, index: idx }">
    ...
    <!--  tree-select 支持导入 -->
    <div v-if="(selectedWidget.type === 'cascader' || selectedWidget.type === 'tree-select' || selectedWidget.type === 'tree')">
      <el-button type="default" @click="importCascaderOptions">{{i18nt('designer.setting.importOptions')}}</el-button>
      <el-button type="default" @click="resetDefault">{{i18nt('designer.setting.resetDefault')}}</el-button>
    </div>
    <!--  transfer 支持导入 -->
    <div v-if="(selectedWidget.type === 'radio') || (selectedWidget.type === 'checkbox') || (selectedWidget.type === 'select') || (selectedWidget.type === 'transfer')">
      <el-button type="default" @click="addOption">{{i18nt('designer.setting.addOption')}}</el-button>
      <el-button type="default" @click="importOptions">{{i18nt('designer.setting.importOptions')}}</el-button>
      <el-button type="default" @click="resetDefault">{{i18nt('designer.setting.resetDefault')}}</el-button>
    </div>
