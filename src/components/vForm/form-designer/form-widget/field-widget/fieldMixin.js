import {deepClone, getDSByName, overwriteObj, runDataSourceRequest, translateOptionItems} from '@vForm/utils/util'
...
export default {
  ...
  computed: {
    ...
    getTreeOptionLabel() { // 按 tree-select值查列表
      if (this.fieldModel === null) {
        return '--'
      } else {
        let resultContent = '--'
        if (!Array.isArray(this.fieldModel)) {
         // console.log(this.fieldModel)
          resultContent = this.gFindInArray(this.field.options.optionItems,this.fieldModel)
        }else{
          this.fieldModel.forEach(sValue=>{
            let ret = this.gFindInArray(this.field.options.optionItems,sValue);
            // console.log(ret)
            if(ret){
               resultContent = resultContent === '--' ? ret : resultContent + ' ' + ret
            }
          })
        }
        return resultContent
    ...
    getTransferOptionLabel() { //transfer 查 结果 key
      if (this.fieldModel === null) {
        return '--'
      } else {
        //console.log(this.fieldModel)
        ...
        let resultContent = '--'
        if (!Array.isArray(this.fieldModel)) { // 不是数组时 直接 按值检索 列表
          resultContent = this.gFindInArray(this.field.options.optionItems,this.fieldModel)
        }else{
          this.field.options.optionItems.forEach(oItem => {
            if ((oItem.key === this.fieldModel) || (oItem.value === this.fieldModel) || (this.findInArray(this.fieldModel, oItem.value)) !== -1 || (this.findInArray(this.fieldModel, oItem.key)) !== -1) {
              resultContent = resultContent === '--' ? oItem.label : resultContent + ' ' + oItem.label
            }
          })
        }
    ...
    getOptionItemSingle(){ // 给出 完整OptionItem节数据
      if (this.fieldModel === null) {
        return []
      } else {
        let resultContent = []
        if (!Array.isArray(this.fieldModel)) { // 不是数组时 直接 按值检索 列表
          resultContent = this.gFindInArrayRealItem(this.field.options.optionItems,this.fieldModel)
        }
        return resultContent
      }
    },
  ...
  methods: {
    /**
     * 从optionItems多维数组中查找
     * @param {array||object} arrayObj 数组或对象
     * @param {*} sValue 数字或字符串等
     * @returns string label 
     */
	  gFindInArray(arrayObj,sValue){
      for(let oItem of arrayObj || []) {
        if(oItem.value == sValue) return oItem.label
        const oItem_ = this.gFindInArray(oItem.children,sValue)
        if(oItem_) return oItem_
      }
    },    
    /**
     * 从多维数组中查找 返回 item
     ...
     * @returns 
     */
    gFindInArrayRealItem(arrayObj,sValue){
      for(let oItem of arrayObj || []) {
        if(oItem.value == sValue) return oItem
        const oItem_ = this.gFindInArrayRealItem(oItem.children,sValue)
        if(oItem_) return oItem_
      }
    },
    ...
    async initOptionItems(keepSelected) {
      ...
      if ((this.field.type === 'radio') || (this.field.type === 'checkbox')
          || (this.field.type === 'select') || (this.field.type === 'transfer') || (this.field.type === 'tree-select') || (this.field.type === 'tree') || (this.field.type === 'cascader')) {
        /* 首先处理数据源选项加载 */
