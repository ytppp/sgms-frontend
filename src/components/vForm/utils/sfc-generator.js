import {isNotNull, generateId} from '@vForm/utils/util'
...
import {
  ...
  formatNumber6,
  formatNumber7,
} from '@vForm/utils/format'
import {emptyAttr} from '@vForm/utils/sfc-util'
...
const elTemplates = {  //字段组件属性
  ...
  'hidden': (widget, formConfig, parentWidget) => {
    const {vModel, clearable, } = getElAttrs(widget, formConfig, parentWidget)
    return `<el-input ${vModel}  ${clearable} style="display:none"></el-input>`
  },
  ...
  // 20230322 FYCDC-Germ  add end
  'tree': (widget, formConfig, parentWidget) => {
    let wop = widget.options
    const {vModel,  size, clearable, filterable, allowCreate, defaultFirstOption,
      multiple,  remote, placeholder} = getElAttrs(widget, formConfig, parentWidget)
    
    const checkStrictly = `check-strictly="${wop.checkStrictly}"`
    const disabled = wop.disabled ? `disabled="${wop.disabled}"` : ''
    const showCheckBox = wop.showCheckBox ? `show-checkbox="${wop.showCheckBox}"` : ''
    // const expandAllable = !wop.expandAllable || `show-checkbox="${wop.showCheckBox}"`
    let treeHtmlStr = '';
    if(filterable) {
      treeHtmlStr += `<el-input v-model="filterText" placeholder="Filter keyword" />`
    }
    treeHtmlStr += `<el-button-group class="ml-4">`
    if(wop.expandAllable){
      treeHtmlStr += `<el-button type="primary"  @click="expandAllNodes(0)">{{i18nt('render.hint.expandOrCollapse')}}</el-button>`
    }
    if(wop.checkAllable){
      treeHtmlStr += `<el-button type="default" v-if="field.options.checkAllable" @click="checkAllNodes(0)">{{i18nt('render.hint.selectAllOrCancel')}}</el-button> `
    }

    treeHtmlStr += `<!-- <el-button type="default"  @click="getCheckedNodes()">get node</el-button>
    <el-button type="default"  @click="getCheckedKeys()">get key</el-button>-->
  </el-button-group>`

    const defaultExpandAll = wop.defaultExpandAll ?  `default-expand-all = "${wop.defaultExpandAll}"` : ''
    const expandOnClickNode = wop.expandOnClickNode ?  `expand-on-click-node=${wop.expandOnClickNode}` : ''
    const multipleLimit = wop.multipleLimit ?  `multiple-Limit=${wop.multipleLimit}` : 'multiple-Limit=0'

    treeHtmlStr += `<el-tree ${vModel}  class="full-width-input" ${disabled} ${showCheckBox} ${defaultExpandAll}  ${expandOnClickNode}  ${checkStrictly} ${size} ${clearable} ${filterable}
            ${allowCreate} ${defaultFirstOption} ${multiple} ${multipleLimit} ${placeholder}
            ${remote}></el-tree>`
    return treeHtmlStr
  },
