  ...
  <div>
    ...
    <el-collapse v-model="activeName" >
        ...
        <el-collapse-item name="Task" v-if="elementType.indexOf('Task') !== -1">
          <template #title><el-icon><Tickets /></el-icon> 任务配置</template>
          <user-task-panel :id="elementId"/>
        </el-collapse-item>

        <!--   表单     -->
        <el-collapse-item name="form" v-if="formVisible">
          <template #title><el-icon><Document /></el-icon> 表单配置</template>
        ...
        <el-collapse-item name="condition" v-if="conditionVisible" >
          <template #title><el-icon><Share /></el-icon> 流转条件</template>
          <condition-panel :id="elementId"/>
        </el-collapse-item>

        <!--   扩展属性     -->
        <el-collapse-item name="properties" >
          <template #title><el-icon><CirclePlus /></el-icon> 扩展属性</template>
